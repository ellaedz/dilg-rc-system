...
@section('title', 'Account Profile - CIVICLEAR')
...
<div class="page-header flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
    <div>
        <div class="dashboard-eyebrow text-[#174ea6]">Account information</div>
...
<div class="grid gap-5 lg:grid-cols-[20rem_minmax(0,1fr)]">
    <aside class="dashboard-panel">
        <div class="dashboard-panel-body text-center">
            <div class="mx-auto grid h-24 w-24 place-items-center rounded-full border-4 border-blue-200 bg-[#102b4c] text-3xl font-black text-[#6dc8ff] shadow-sm">{{ strtoupper(substr($user->name ?: $user->email, 0, 1)) }}</div>
            <h2 class="mt-4 text-lg font-extrabold text-slate-900">{{ $user->name }}</h2>
            <p class="mt-1 text-sm font-semibold text-[#174ea6]">{{ $isBarangay ? 'Barangay Staff' : 'DILG Administrator' }}</p>
        ...
        <section class="dashboard-panel">
            ...
            <div class="dashboard-panel-body">
                <dl class="grid gap-5 sm:grid-cols-2">
                    <div><dt class="text-xs font-extrabold uppercase tracking-wide text-slate-500">Account name</dt><dd class="mt-1.5 font-semibold text-slate-800">{{ $user->name }}</dd></div>
